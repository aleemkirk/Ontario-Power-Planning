"""
Visualize capacity expansion timelines for different Pareto solutions.

Creates graphs showing when to build each type of power plant over 20 years
for three scenarios: emissions-optimal, balanced, and cost-optimal.

Usage:
    python visualize_capacity_expansion.py
"""
...
def run_and_extract_builds(alpha, start_year=2025, end_year=2045):
    """
    Run optimization for given alpha and extract detailed build schedule.
    ...
    """
    print(f"\n{'='*80}")
    print(f"Running optimization: α = {alpha}")
    print(f"{'='*80}")
...
def visualize_expansion_timeline(builds_data, alpha_values, output_dir='results/figures'):
    ...
    fig.suptitle('Capacity Expansion Timeline: When to Build Power Plants (2025-2045)',
                 fontsize=16, fontweight='bold', y=0.995)

    scenario_names = {
        0.0: 'Emissions-Optimal (α=0.0)',
        0.5: 'Balanced (α=0.5)',
        1.0: 'Cost-Optimal (α=1.0)'
    }
    ...
    for idx, alpha in enumerate(alpha_values):
        ...
        # Group by year and plant type, sum capacity
        ...
        # Highlight years with significant builds
        max_year_build = pivot_data.sum(axis=1).idxmax()
        max_build = pivot_data.sum(axis=1).max()
        if max_build > 0:
            year_idx = list(pivot_data.index).index(max_year_build)
            ax.axvline(x=year_idx, color='red', linestyle=':', alpha=0.5, linewidth=2)
            ax.text(year_idx, max_build * 1.05, f'Peak: {max_build:.0f} MW',
                   ha='center', fontsize=9, color='red', fontweight='bold')

    plt.tight_layout()

    # Save figure
    fig_file = output_path / 'capacity_expansion_timeline.png'
...
def create_cumulative_capacity_plot(builds_data, alpha_values, output_dir='results/figures'):
    ...
    fig, axes = plt.subplots(3, 1, figsize=(14, 12))
    fig.suptitle('Cumulative Capacity Evolution (2025-2045)',
                 fontsize=16, fontweight='bold', y=0.995)
    ...
    for idx, alpha in enumerate(alpha_values):
        ...
        # Add final capacity annotation
        final_total = cumulative_df.iloc[-1].sum()
        ax.text(0.02, 0.98, f'Final Total: {final_total/1000:.1f} GW',
               transform=ax.transAxes, fontsize=10, fontweight='bold',
               verticalalignment='top',
    ...
    # Save figure
    fig_file = output_path / 'cumulative_capacity_evolution.png'
    plt.savefig(fig_file, dpi=300, bbox_inches='tight')
...
def create_build_comparison_table(builds_data, alpha_values):
    ...
    print("\n" + "="*80)
    print("NEW CAPACITY BUILT BY PLANT TYPE (GW)")
    print("="*80)
    print(comparison_df.to_string(float_format=lambda x: f'{x:>8.2f}'))
    print("="*80)

    # Save to CSV
    output_file = Path('results/data') / 'capacity_builds_comparison.csv'
...
if __name__ == '__main__':
    print("="*80)
    print("CAPACITY EXPANSION TIMELINE VISUALIZATION")
    print("="*80)
    ...
    print("="*80)
    print("\nGenerated files:")
    print("  • results/figures/capacity_expansion_timeline.png")
    print("  • results/figures/cumulative_capacity_evolution.png")
    print("  • results/data/capacity_builds_comparison.csv")
    print("\n" + "="*80)
